...
A few sections ago, we discussed what we called 'nice' functions.  These were functions that, at a particular $x$-value $a$, had easy-to-compute limits.  In fact, computing the limit of a 'nice' function is as simple as plugging in $a$ into the function itself so that $ \lim_{x\to a} f(x) = f(a).$
...
\begin{theorem}[Continuity of Common Functions]\index{continuity of common functions}\label{theorem:continuity}

...
\begin{warning}
Notice that this theorem starts by supposing that $\displaystyle\lim_{x\to a}f(x)$ and $\displaystyle\lim_{x\to a}g(x)$ exist.  Without this supposition, none of the limit laws are necessarily true!  Be careful to not use a limit law when either $\displaystyle\lim_{x\to a}f(x)$ or $\displaystyle\lim_{x\to a}g(x)$ do not exist.
\end{warning}

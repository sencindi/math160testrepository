...
\begin{question}
  ...
  \begin{question}
    ...
    \begin{feedback}
      ...
      the limit of the denominator does not equal $0$, we see that we can apply the Quotient Law to start evaluating this limit.  After that, you can use some of the other limit laws in order to finish evaluating the limit.  Notice that ultimately, this limit could be evaluated by 'plugging in' $x=2$ into the function.  In other words, if $f(x) = \frac{x^2+3x+2}{x+2}$, then $$\displaystyle\lim_{x\to 2} f(x) = f(2) = 3,$$ and it turns out that $f(x)$ is a 'nice' function at $x=2$!  
    \end{feedback}
  \end{question}
\end{question}

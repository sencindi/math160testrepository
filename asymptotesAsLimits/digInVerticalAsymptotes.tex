...
%% \label{plot:1/(x+1)^2}
\end{image}
As we stated learned on the previous card, $\lim_{x\to -1} f(x)$ does not exist, but something can still be said. 
...
If \textit{at least} one of the following hold:
\begin{itemize}
...
From the one-sided limit information, we can conclude that $\displaystyle\lim_{x\to 3} f(x)$ does not exist.  Notice that $f(x)$ still has a vertical asymptote at $x=3$ despite the fact that this limit does not exist.
\end{explanation}
\end{example}

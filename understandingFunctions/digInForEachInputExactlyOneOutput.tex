...
it is described.  \\ For example, if $f(x) = x^2$ and $g(u) = u^2$, like on the previous card, $f = g$: although these are two different formulas, they describe the same relation, so they are equal functions.
\end{warning}

...
To use this test, imagine passing a vertical line left to right over the entire domain of the graph of $y=f(x)$.  If there is a vertical line $x=a$ that intersects the curve $y=f(x)$ \textit{more than once}, then there is an input ($a$) that corresponds to more than one output, so the graph is \textit{not} the graph of a function.  On the other hand, if a vertical line can be moved through the entire domain of your function and \textit{always} intersects the curve $y=f(x)$ exactly once, then each input corresponds with exactly one output, so $f(x)$ is a function.

...
\begin{explanation}
Notice that in the previous two examples, we were able to use algebra to manipulate the limits we were dealing with.  Instead of dealing with the more difficult $\lim_{x\to1}\frac{x-1}{x^2+2x-3}$, we were able to factor and conclude that $\lim_{x\to1}\frac{x-1}{x^2+2x-3} = \lim_{x\to1}\frac{1}{x+3}$ and from there we can use the limit laws to evaluate the original limit.  The idea is to use algebra techniques (like factoring) to reduce a more difficult limit problem to one that we can solve with the techniques we already have.
\end{explanation}

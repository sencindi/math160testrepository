...
using the limit laws and the Squeeze Theorem. What about limits that cannot be
...
Let's consider an example with the function above.  This next example will require that you can factor a quadratic polynomial.  If you have forgotten how to do this or would like to brush up on this skill, go \href{https://ximera.osu.edu/math160fa17/m160prerequisites/PreRequisiteXards/U6MultiplyingAndFactoringPolynomials/6.2FactoringPolynomials/titlePage}{here}.
...
\begin{warning}
Although $\lim_{x\to 2}\frac{x^2-3x+2}{x-2} = \lim_{x\to 2} (x-1)$, it is not true that $\frac{x^2-3x+2}{x-2} = x-1!$  
\end{warning}
...
\begin{explanation}
Notice that in the previous two examples, we were able to use algebra to manipulate the limits we were dealing with.  Instead of dealing with the more difficult $\lim_{x\to1}\frac{x-1}{x^2+2x-3}$, we were able to factor and conclude that $\lim_{x\to1}\frac{x-1}{x^2+2x-3} = \lim_{x\to1}\frac{1}{x+3}$ and from here we could use the limit laws to evaluate the original limit.  The idea is to use algebra techniques (like factoring) to reduce a more difficult limit problem to one that we can solve with the techniques we already have.
\end{explanation}

The next example will involve subtracting and dividing two rational functions.  If you are unsure on how to do this or would like to review this skill, you should check out \href{https://ximera.osu.edu/math160fa17/m160prerequisites/PreRequisiteXards/U3Fractions/3.1ArithmeticAndAlgebraOfFractions/3.1TitlePage/3.1TitlePage}{this card} before moving forward.  
...
\begin{explanation}
  ...
  numerator, let's subtract the two fractions in the numerator: 
  \[
  \frac{1}{x+1}-\frac{3}{x+5} = \frac{x+5}{(x+5)(x+1)} - \frac{3(x+1)}{(x+5)(x+1)} = \frac{x+5 - 3(x+1)}{(x+5)(x+1)} = \frac{-2x + 2}{(x+5)(x+1)}
  \]

so that 

\[ \frac{\frac{1}{x+1}-\frac{3}{x+5}}{x-1} = \frac{\frac{-2x + 2}{(x+5)(x+1)}}{x-1} =  \frac{-2x+2}{(x+1)(x+5)(x-1)}\]

Now, we would like to factor an $x-1$ from both the numerator and denominator.  If you look closely, you'll see that $-2x+2 = \answer{-2}(x-1)$.  Therefore, 
...
So once again, with a little bit of algebra, we were able to re-write the original limit as one that can be easily solved by applying the usual Limit Laws.  Hence
\begin{align*}
...
If you are struggling to multiply $\left(\sqrt{x+5}-2\right)$ and $\left(\sqrt{x+5}+2\right)$, you may want to review \href{https://ximera.osu.edu/math160fa17/m160prerequisites/PreRequisiteXards/U6MultiplyingAndFactoringPolynomials/6.1MultiplyingPolynomials/6.1TitlePage/6.1TitlePage}{how to multiply polynomials}.  It might also help if you write $\sqrt{x+5}$ as $(x+5)$ raised to an exponent. 
